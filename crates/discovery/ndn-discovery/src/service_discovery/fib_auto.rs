//! FIB auto-population and TTL expiry.
...
impl ServiceDiscoveryProtocol {
    pub(super) fn auto_populate_fib(
        ...
        // Clamp to config bounds so a forged `freshness_ms` can't pin a
        // route indefinitely (or expire it instantly at 0) (audit #2).
        let raw_ttl =
            (record.freshness_ms as f64 * self.config.auto_fib_ttl_multiplier as f64) as u64;
        let ttl_ms = raw_ttl.clamp(
            self.config.auto_fib_min_ttl.as_millis() as u64,
            self.config.auto_fib_max_ttl.as_millis() as u64,
        );
    ...
    /// Also drops `peer_records` and resets `browsed_neighbors` for the
    /// affected node so a re-browse fires on the next tick.
