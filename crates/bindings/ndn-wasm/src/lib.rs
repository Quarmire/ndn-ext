//! WebAssembly bindings for the in-browser NDN simulator.
//!
//! - [`WasmPipeline`] — single-node simulation backing the Pipeline view.
//! - [`WasmTopology`] — multi-node simulation backing the Topology view.
//! - `tlv_*` functions — stateless TLV encode/decode for the Packet Explorer.
...
/// Single-node simulation: FIB / PIT / CS state plus pipeline execution that
/// emits trace events back to JS as JSON.
#[wasm_bindgen]
...
impl WasmPipeline {
    ...
    /// Advance the simulated clock and evict expired PIT entries.
    ...
    /// `strategy` is one of `"BestRoute" | "Multicast" | "Suppress"`.
    ...
    /// Run an Interest. Returns a JSON-encoded `PipelineTrace`. A `nonce` of
    /// 0 is replaced by a clock-derived pseudorandom value.
    ...
    /// Run a Data packet. Returns a JSON-encoded `PipelineTrace`.
    pub fn process_data(
        &mut self,
        name: &str,
        content: &str,
    ...
    /// Reset PIT, CS, and measurements; FIB and config are preserved.
    pub fn reset(&mut self) {
        self.inner.reset();
    }
}

/// Multi-node simulation; same pipeline as [`WasmPipeline`] but propagates
/// Interest/Data across links and emits per-hop trace events.
#[wasm_bindgen]
...
impl WasmTopology {
    ...
    /// Populate every node's FIB by BFS from each producer.
    pub fn auto_configure_fib(&mut self) {
        self.inner.auto_configure_fib();
    }

    /// Send an Interest from `from_node`; returns a JSON-encoded `TopologyTrace`.
    ...
    /// Reset PIT/CS/measurements; nodes and links survive.
...
/// Encode an Interest as a space-separated hex string.
#[wasm_bindgen]
...
/// Encode a Data as a space-separated hex string.
#[wasm_bindgen]
...
/// Parse hex (with or without spaces) and return a JSON-encoded `Vec<TlvNode>`.
#[wasm_bindgen]
...
/// `scenario` is one of `"linear" | "triangle-cache" | "multipath" |
/// "aggregation" | "discovery" | "link-failure"`. Returns a JSON description
/// of the constructed topology.
#[wasm_bindgen]
...
fn scenario_multipath(topo: &mut SimTopology) -> ScenarioDesc {
    ...
    let l2 = topo.add_link(router, producer, 10, 1_000_000, 0.0);
    // Demo uses two links to the same producer.
