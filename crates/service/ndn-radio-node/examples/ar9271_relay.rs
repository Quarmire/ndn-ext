...
//! its `FrameIo` is wrapped in a [`MonitorWifiFace`] (the same LP link-service path the RTL8822E relay
...
use ndn_face_monitor_wifi::{FaceId, MonitorWifiFace, open_ath9k};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("AR9271 open on ch{ch}; wrapping its FrameIo in a MonitorWifiFace (engine face {})", RADIO_FACE_ID.0);

    // Compose the engine: an in-proc app face + the AR9271 radio face.
    let (app_face, app_handle) = InProcFace::new(APP_FACE_ID, 64);
    let radio_face = MonitorWifiFace::new(FaceId(RADIO_FACE_ID.0), io).into_face();
    ...
    println!("FIB: /radio-demo → AR9271 face. Expressing {ticks} Interests (each forwards on air)…");
    ...
    for seq in 0..ticks {
        ...
        if let Ok(Some(wire)) = tokio::time::timeout(Duration::from_millis(tick_ms), app_handle.recv()).await {
            if let Ok(d) = Data::decode(wire) {
                got += 1;
                println!("  seq {seq}: Data over the AR9271 ({} B) ✓", d.content().map(|c| c.len()).unwrap_or(0));
            }
        }
    }
