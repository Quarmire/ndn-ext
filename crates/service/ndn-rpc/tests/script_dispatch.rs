...
async fn untyped_script_service_round_trips_over_a_carrier() {
    ...
    let shout: ScriptHandler = Arc::new(|req: Bytes| {
        Ok(Bytes::from(
            String::from_utf8_lossy(&req).to_uppercase().into_bytes(),
        ))
    });
    ...
    let r = carrier
        .invoke(&svc, &OpId::new("echo"), Bytes::from_static(b"hi"))
        .await
        .unwrap();
    assert_eq!(r.payload.as_ref(), b"hi");
    let r = carrier
        .invoke(&svc, &OpId::new("shout"), Bytes::from_static(b"hi"))
        .await
        .unwrap();
    assert_eq!(r.payload.as_ref(), b"HI");
    ...
    assert!(
        carrier
            .invoke(&svc, &OpId::new("nope"), Bytes::new())
            .await
            .is_err()
    );
}
...
async fn panicking_script_handler_is_isolated() {
    ...
    assert!(
        carrier
            .invoke(&svc, &OpId::new("boom"), Bytes::from_static(b"x"))
            .await
            .is_err()
    );
    // A subsequent op still works — the service survived the panic.
    let r = carrier
        .invoke(&svc, &OpId::new("echo"), Bytes::from_static(b"ok"))
        .await
        .unwrap();
